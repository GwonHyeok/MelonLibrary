...
//			songinfoelements.select("dd").get(3).ownText() 기획사 
			Document doc = Jsoup.connect("http://www.melon.com/album/detail.htm?albumId="+SongData[5]).get();
	...
	public boolean isBanSong() {
		...
		return error[0].equals("-1007") ? true : false;
	}
	...
	public String getStringLyrics() {
		...
		try {
			...
			String lyrics = LyricsElements.get(0).html().replaceAll("<br />", "").split("> ")[1];
			return lyrics;
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public String getHtmlLyrics() {
		...
		try {
			Document doc = Jsoup.connect(url).get();
			Elements LyricsElements = doc.getElementsByClass("lyric");
			String lyrics = "<html>"+LyricsElements.get(0).html().replaceAll("<br />", "<br>").split("> ")[1]+"</html>";
			return lyrics;
		} catch (IOException e) {
			e.printStackTrace();
		}
